 ...
 */
public class ScoreCircleCenterView extends FrameLayout {
    ...
    public ScoreCircleCenterView(Context context) {
        super(context);
        init();
    }

    public ScoreCircleCenterView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ScoreCircleCenterView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public ScoreCircleCenterView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init() {
        ...
        addView(LayoutInflater.from(getContext()).inflate(R.layout.widget_score_circle_center, this, false));
        mHolder = new ViewHolder(this);
    }

    public void setScore(Score score, StateType type) {
        if (score == null) {
            mCurrentScore = Score.EMPTY;
        } else {
            mCurrentScore = score;
        }
        mHolder.description.setText(type.getTitleRes());
        mHolder.score.setText(mCurrentScore.getScoreText());
        invalidate();
    }
